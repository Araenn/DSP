...
alpha = 0.7; %constante debruitage
count = 0;
audio_out = zeros(N, 1);
...
while ~isDone(myReader)
    ...
    if count <= 2*Fs
        estimation_spectre_bruit = mean(abs(fft(signal_entree_pondere)).^2); %spectre estimé du bruit
    else
        
        if mean(spectre_entree_pondere)/estimation_spectre_bruit > 1
            
            if (mean(spectre_entree_pondere) - estimation_spectre_bruit)/(2*estimation_spectre_bruit) > (1 - estimation_spectre_bruit)
                
                module_signal_debruite = max(spectre_entree_pondere - alpha*estimation_spectre_bruit, 0);
                
            else
                estimation_spectre_bruit = mean(abs(fft(signal_entree_pondere)).^2);
                module_signal_debruite = max(spectre_entree_pondere - alpha*estimation_spectre_bruit, 0);
            end
